#!/usr/bin/env python3
"""
Detect an ArUco marker with an Azure Kinect camera and publish
...
from azure_kinect_camera import get_bgr_frame, get_camera_intrinsics, start_azure_kinect
...
class QRPublisher(Node):
    def __init__(self, args):
        ...
        self.camera = start_azure_kinect(
            device_id=0,
            width=args.width,
            height=args.height,
            fps=args.fps,
    ...
    def detect_marker(self, img_bgr):
        if self.K is None:
            ...
            self.get_logger().info("Intrinsics auto-fetched from Azure Kinect")
...
def main():
    ap = argparse.ArgumentParser(description="Publish base->QR TF using Azure Kinect & ArUco (ROS 2)")
    ap.add_argument("--intrinsics", default=None, help="Optional YAML with camera_matrix & dist_coeff")
    ...
    ap.add_argument("--cam-frame", default="azure_kinect_camera")
